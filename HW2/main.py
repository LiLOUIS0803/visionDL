...
def make_mosaic(img_dir, img_infos, anns_list, target_size):
    """
    4-image mosaic. Place 4 images into a target_size x target_size canvas.
...
def collate_fn_test(batch):
    ...
    return (
        ...
        list(ids), list(hs), list(ws), list(scales), list(pls), list(pts),
    )
...
class MSDeformableAttention(nn.Module):
    ...
    def forward(self, query, reference_points, value, spatial_shapes):
        ...
        offsets = self.sampling_offsets(query).view(
            bsz, len_q, self.n_heads, self.n_levels, self.n_points, 2
        )
        attn_weights = self.attention_weights(query).view(
            bsz, len_q, self.n_heads, self.n_levels * self.n_points
        )
        ...
        for lid, (h, w) in enumerate(spatial_shapes):
            value_l = value_list[lid].permute(0, 2, 3, 1).reshape(
                bsz * self.n_heads, self.head_dim, h, w
            )
            grid_l = sampling_grids[:, :, :, lid, :, :].permute(0, 2, 1, 3, 4).reshape(
                bsz * self.n_heads, len_q, self.n_points, 2
            )
        ...
        attn_weights = attn_weights.permute(0, 2, 1, 3).reshape(
            bsz * self.n_heads, 1, len_q, self.n_levels * self.n_points
        )
        output = (sampled_values * attn_weights).sum(dim=-1)
        output = (
            output.view(bsz, self.n_heads, self.head_dim, len_q)
            .permute(0, 3, 1, 2)
            .reshape(bsz, len_q, self.d_model)
        )
...
def make_denoising_queries(
    targets, num_classes, dn_number, label_noise_ratio, box_noise_scale, device
):
    if dn_number <= 0:
        return None
        ...
        for rep in range(dn_number):
            ...
            if label_noise_ratio > 0:
                ...
                noisy_labels = torch.where(
                    mask,
                    torch.randint(1, num_classes + 1, (num_gt,), device=device),
                    noisy_labels,
                )
...
class DeformableDETR(nn.Module):
    def __init__(
        ...
        self.encoder_layers = nn.ModuleList([
            DeformableEncoderLayer(hidden_dim, nheads, n_levels, n_points, dim_feedforward, dropout)
            for _ in range(enc_layers)
        ])
        self.decoder_layers = nn.ModuleList([
            DeformableDecoderLayer(hidden_dim, nheads, n_levels, n_points, dim_feedforward, dropout)
    ...
    def _build_dn_inputs(self, targets, device):
        ...
        out = make_denoising_queries(
            targets, self.num_classes, self.dn_number,
        ...
        return (
            self.label_enc(dn_labels) + self.box_enc(dn_boxes),
            self.dn_query_pos(dn_boxes),
            dn_boxes,
            dn_meta,
        )
    ...
    def forward(self, x, targets=None):
        ...
        for lid, feat in enumerate([c3, c4, c5, c5]):
            ...
            pos = (
                build_2d_sincos_position_embedding(h, w, self.hidden_dim, device)
                .unsqueeze(0)
                .expand(bsz, -1, -1)
        ...
        enc_ref = (
            self._get_encoder_reference_points(spatial_shapes, device)
            .unsqueeze(0)
            .expand(bsz, -1, -1, -1)
        )
        memory = src_flat
        ...
        qp, qc = qe.split(self.hidden_dim, dim=-1)
        query_pos = qp.unsqueeze(0).expand(bsz, -1, -1)
        tgt = qc.unsqueeze(0).expand(bsz, -1, -1)
        ...
        for lid, layer in enumerate(self.decoder_layers):
            output = layer(
                ...
                query_pos,
                memory,
                reference_points[:, :, None, :2].repeat(1, 1, self.n_levels, 1),
                spatial_shapes,
                attn_mask=total_attn_mask,
            )
            output_norm = self.decoder_norm(output)
            pred_boxes = (
                self.bbox_heads[lid](output_norm) + inverse_sigmoid(reference_points)
            ).sigmoid()
            outputs_classes.append(self.class_heads[lid](output_norm))
    ...
    def forward(self, outputs, targets, use_focal=False):
        ...
        for b in range(bsz):
            ...
            cost_giou = -generalized_box_iou(
                box_cxcywh_to_xyxy(pred_box), box_cxcywh_to_xyxy(tgt_boxes)
            )
            cost = (
                self.cost_class * cost_class
                + self.cost_bbox * cost_bbox
            ...
            indices.append(
                (torch.as_tensor(r, dtype=torch.long), torch.as_tensor(c, dtype=torch.long))
            )
        return indices
...
class SetCriterion(nn.Module):
    ...
    def _compute_loss(self, outputs, targets, indices):
        ...
        if self.use_focal:
            ...
            loss_ce = sigmoid_focal_loss(
                outputs["pred_logits"], tgt_oh, self.focal_alpha, self.focal_gamma, num_boxes
            )
        else:
            tc = torch.zeros(outputs["pred_logits"].shape[:2], dtype=torch.long, device=device)
            for b, (si, ti) in enumerate(indices):
                if len(si) > 0:
                    tc[b, si] = targets[b]["labels"][ti]
            loss_ce = F.cross_entropy(
                outputs["pred_logits"].permute(0, 2, 1), tc, weight=self.empty_weight
            )

        sb, tb = [], []
        for b, (si, ti) in enumerate(indices):
            if len(si) > 0:
                sb.append(outputs["pred_boxes"][b][si])
                tb.append(targets[b]["boxes"][ti])
        if sb:
            sb = torch.cat(sb)
            tb = torch.cat(tb)
            loss_bbox = F.l1_loss(sb, tb, reduction="sum") / num_boxes
            loss_giou = (
                (1.0 - generalized_box_iou(box_cxcywh_to_xyxy(sb), box_cxcywh_to_xyxy(tb)).diag())
                .sum()
                / num_boxes
            )
        else:
    ...
    def _compute_dn_loss(self, outputs, targets, dn_meta):
        ...
        pl = outputs["pred_logits"][:, :pad_size]
        pb = outputs["pred_boxes"][:, :pad_size]
        sl, tl, sbox, tbox, sid_list = [], [], [], [], []
        for b, pairs in enumerate(dn_meta["dn_positive_idx"]):
            if not pairs:
                sid_list.append(torch.empty(0, dtype=torch.long, device=device))
                continue
            ...
            sid_list.append(si)
            sl.append(pl[b, si])
            tl.append(targets[b]["labels"][ti].to(device))
            sbox.append(pb[b, si])
            tbox.append(targets[b]["boxes"][ti].to(device))
        if not sl:
            return torch.tensor(0.0, device=device), 0.0
        nb = max(sum(s.numel() for s in sid_list), 1)
        if self.use_focal:
            B, _, C = pl.shape
            tgt_oh = torch.zeros_like(pl)
        ...
        lg = (
            (1.0 - generalized_box_iou(
                box_cxcywh_to_xyxy(sbox), box_cxcywh_to_xyxy(tbox)
            ).diag()).sum()
            / nb
        )
    ...
    def _split(self, outputs):
        ...
        ps = int(dm["pad_size"])
        mo = {
            "pred_logits": outputs["pred_logits"][:, ps:],
            "pred_boxes": outputs["pred_boxes"][:, ps:],
        }
        if "aux_outputs" in outputs:
            mo["aux_outputs"] = [
                {
                    "pred_logits": a["pred_logits"][:, ps:],
                    "pred_boxes": a["pred_boxes"][:, ps:],
                }
    ...
    def forward(self, outputs, targets):
        ...
        if "aux_outputs" in mo:
            for aux in mo["aux_outputs"]:
                ...
                al, _, _, _ = self._compute_loss(aux, targets, ai)
                la = la + al
        ld = torch.tensor(0.0, device=lm.device)
        dv = 0.0
        if dm is not None:
            ld, dv = self._compute_dn_loss(outputs, targets, dm)
        total = lm + la + ld
        return (
            total,
            {
                "loss": float(total.item()),
                "loss_main": float(lm.item()),
                "loss_aux": float(la.item()),
                "loss_ce": ce,
                "loss_bbox": bb,
                "loss_giou": gi,
                "loss_dn": dv,
...
def postprocess_single_image_predictions(
    ...
    orig_w, orig_h, score_thresh, nms_thresh, use_focal=False,
):
    if use_focal:
    ...
    ki = torch.cat(ki)
    return [
        {"bbox": bx[i].tolist(), "score": float(scores[i]), "category_id": int(cls_ids[i])}
        for i in ki.tolist()
    ]
...
@torch.no_grad()
def evaluate(
    model, loader, criterion, device, img_size, output_dir, epoch,
    ...
    for imgs, targets in tqdm(loader, desc="Eval", leave=False):
        ...
        with get_autocast(device, enabled=use_amp):
            outputs = model(imgs)
            _, ld, indices = criterion(outputs, targets)
        ml = outputs["pred_logits"]
        if "dn_meta" in outputs:
            ml = ml[:, int(outputs["dn_meta"]["pad_size"]):]
        for k, v in ld.items():
            tl[k] += float(v)
        nb += 1
        if use_focal:
            pc = ml.sigmoid().argmax(dim=-1) + 1
        else:
            pc = ml.argmax(dim=-1)
        for b, (si, ti) in enumerate(indices):
            if len(si) > 0:
                apc.extend(pc[b][si].cpu().tolist())
                agc.extend(targets[b]["labels"][ti].cpu().tolist())
        pb = outputs["pred_boxes"]
        if "dn_meta" in outputs:
            pb = pb[:, int(outputs["dn_meta"]["pad_size"]):]
        for b in range(imgs.size(0)):
            iid = int(targets[b]["image_id"])
            oh, ow = targets[b]["orig_size"].tolist()
            sc = float(targets[b]["scale"])
            pl, pt = targets[b]["pad"].tolist()
            ag["images"].append({"id": iid, "width": int(ow), "height": int(oh)})
            for j in range(len(targets[b]["labels"])):
                ba = convert_to_orig_coords(
                    targets[b]["boxes"][j:j + 1].cpu(), img_size, sc, pl, pt, ow, oh
                )[0].tolist()
                ag["annotations"].append({
                    "id": gid,
                    "image_id": iid,
                    "category_id": int(targets[b]["labels"][j]),
                    "bbox": ba,
                    "area": float(ba[2] * ba[3]),
                    "iscrowd": 0,
                })
                gid += 1
            preds = postprocess_single_image_predictions(
                ml[b].cpu(), pb[b].cpu(), img_size, sc, pl, pt, ow, oh,
    ...
    try:
        ...
        if ap:
            cd = cg.loadRes(ap)
            ce = COCOeval(cg, cd, "bbox")
            with contextlib.redirect_stdout(io.StringIO()):
                ce.evaluate()
                ce.accumulate()
            ce.summarize()
            mAP = float(ce.stats[0])
...
def plot_confusion_matrix(preds, gts, output_dir, epoch, num_classes):
    try:
        ...
        ConfusionMatrixDisplay(cm, display_labels=[str(i) for i in labels]).plot(
            ax=ax, cmap="Blues", values_format="d"
        )
        ax.set_title(f"Confusion Matrix (epoch {epoch + 1})")
        plt.tight_layout()
        plt.savefig(
            os.path.join(output_dir, f"confusion_matrix_ep{epoch + 1}.png"), dpi=150
        )
        plt.close(fig)
    except ImportError:
        pass
...
def plot_curves(history, output_dir):
    ...
    if va:
        axes[1, 0].plot(ea, va, label="Val Acc", marker="o", markersize=3)
...
def inference(model, test_dir, img_size, device, score_thresh, nms_thresh, output_path, use_focal):
    ...
    for imgs, img_ids, hs, ws, scales, pls, pts in tqdm(loader, desc="Inference"):
        ...
        with get_autocast(device, enabled=use_amp):
            outputs = model(imgs)
        preds = postprocess_single_image_predictions(
            outputs["pred_logits"][0].cpu(), outputs["pred_boxes"][0].cpu(),
            img_size, scales[0], pls[0], pts[0], ws[0], hs[0],
            score_thresh, nms_thresh, use_focal,
        )
        for p in preds:
            results.append({
...
def main():
    ...
    print(
        f"Device: {device} | img_size={args.img_size}"
        f" | use_dn={args.use_dn} | use_focal={args.use_focal}"
    )
    print(
        f"Mosaic p={args.mosaic_p}"
        f" | Multi-scale={args.multi_scale} | RandomErase p={args.random_erase_p}"
    )
    ...
    if args.resume:
        ...
        if "best_mAP" in ckpt:
            best_mAP = float(ckpt["best_mAP"])
        if "history" in ckpt:
            history = ckpt["history"]

    if args.do_train:
        train_ds = CocoDigitDataset(
            args.train_img_dir, args.train_ann, args.img_size,
            is_train=True, mosaic_p=args.mosaic_p, random_erase_p=args.random_erase_p,
        )
        ...
        param_dicts = [
            {
                "params": [
                    ...
                    if not n.startswith(bp) and p.requires_grad
                ],
                "lr": args.lr,
            },
            {
                "params": [
                    p for n, p in model.named_parameters()
                    if n.startswith(bp) and p.requires_grad
                ],
                "lr": args.lr_backbone,
            },
        ]
        ...
        scheduler = build_warmup_cosine_scheduler(
            optimizer, args.warmup_epochs, args.epochs, args.min_lr_ratio
        )
        use_amp = device.type == "cuda"
        ...
        matcher = HungarianMatcher(
            args.cost_class, args.cost_bbox, args.cost_giou, args.focal_alpha, args.focal_gamma
        )
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            # ---- Reset to default img_size before validation ----
            train_ds.set_img_size(args.img_size)

            if (epoch + 1) % args.eval_every == 0 or epoch == args.epochs - 1:
                vl, va, vm = evaluate(
                    ema.ema, val_loader, criterion, device, args.img_size, args.output_dir,
                    epoch, args.num_classes, args.val_score_thresh, args.nms_thresh, args.use_focal,
                )
                history["val_loss"].append(vl.get("loss", 0.0))
                history["val_acc"].append(va)
                history["val_mAP"].append(vm)
                print(
                    f"Epoch {epoch + 1} | Train: {atl:.4f}"
                    f" | Val: {vl.get('loss', 0.0):.4f}"
                    f" | Acc: {va:.4f} | mAP(EMA): {vm:.4f}"
                )
                if vm > best_mAP:
                    best_mAP = vm
                    torch.save(
    ...
    if args.do_infer:
        ...
        inference(
            ema.ema, args.test_img_dir, args.img_size, device,
